...
let bonusObstacle = new Obstacle({x: canvas.width, y: 200, width: 100, height: 100});

let bonusObstacles = [bonusObstacle]
...
function runGame() {
  ...
  bonusObstacle.traverse(speed, direction);
  ufo.draw();
  ...
  bonusObstacle.draw(); //replace this line with drawImage,
  // context.drawImage(bonus_image, bonusObstacle.x, bonusObstacle.y)
...
function checkScoreForWin() {
  if (score.currentScore(parsecs) >= 2000) { stopGame(); winGame(); }
}
