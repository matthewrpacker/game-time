...
function saveGameScores() {
  localStorage.setItem(
    'storedScores',
    `${localStorage.storedScores}, ${score.currentScore(parsecs)}`
  );
}

function appendScores(id) {
  $(id).append(`<p>Final Score: ${score.currentScore(parsecs)}</p>`);
}
...
function checkScoreForWin() {
  if (score.currentScore(parsecs) >= 150) { stop(); winGame(); }
}
